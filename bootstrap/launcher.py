...
This keeps the EXE itself tiny and lets you update main.py from GitHub
...
def install_python() -> None:
    ...
    subprocess.check_call([str(PY_EXE), str(getpip)])
...
def run_app() -> int:
    ...
    env = os.environ.copy()
    return subprocess.call([str(PY_EXE), str(main_py)], env=env)
