...
Uses GLM (z-ai-web-sdk compatible) - NO external dependencies
"""
...
DEFAULT_CONFIG = {
    ...
    "api_hash": "",
    "phone": "",
    "text_model": "glm-4-flash",      # GLM текстовая модель
    "vision_model": "glm-4v-flash",    # GLM vision модель
...
# ===========================================================================
# Z-AI CLIENT - GLM API (z-ai-web-sdk compatible)
# ===========================================================================
class ZAIClient:
    """
    GLM AI Client - совместимый с z-ai-web-sdk
    Работает напрямую через HTTP API без Node.js зависимостей
    """
    
    # GLM API endpoint (z-ai-web-sdk использует этот endpoint)
    API_BASE = "https://open.bigmodel.cn/api/paas/v4"
    
    # Доступные модели
    TEXT_MODELS = ["glm-4-flash", "glm-4", "glm-4-plus", "glm-4-air"]
    VISION_MODELS = ["glm-4v-flash", "glm-4v"]

    def __init__(self, config: dict):
        self.config = config
        self.text_model = config.get("text_model", "glm-4-flash")
        self.vision_model = config.get("vision_model", "glm-4v-flash")
        # API ключ не нужен для z-ai-web-sdk (автоматическая авторизация)

    async def chat(self, messages: list) -> str:
        """Chat completion via GLM API"""
        now = datetime.now()
        ...
        try:
            async with httpx.AsyncClient(timeout=60) as client:
                ...
                if response.status_code == 200:
                    ...
                    print(f"GLM API error: {response.status_code} - {error}")
    ...
    async def generate_image(self, prompt: str, size: str = "1024x1024") -> Optional[str]:
        """Generate image via GLM (если поддерживается)"""
        # GLM может не поддерживать генерацию изображений напрямую
...
class BotManager:
    ...
    def __init__(self, config: dict, message_callback, ai_client: ZAIClient):
        self.config = config
...
class NinjaApp:
    def __init__(self):
        ...
        self._log("System", "🚀 Готов к работе (GLM AI)", "system")
    ...
    def _setup_ctk(self):
        ...
        ctk.CTkLabel(h, text="GLM AI", text_color="#10b981", font=("", 11)).pack(side="left")
        ...
        ctk.CTkLabel(s, text="🤖 GLM AI Models", font=("", 12, "bold"), text_color="#10b981").pack(anchor="w", pady=(15, 5))
        
        r3 = ctk.CTkFrame(s, fg_color="transparent")
        r3.pack(fill="x", pady=2)
        ctk.CTkLabel(r3, text="Text Model:", width=90).pack(side="left")
        self.text_model = ctk.CTkComboBox(r3, values=ZAIClient.TEXT_MODELS, width=200)
        self.text_model.pack(side="left", padx=10)
        self.text_model.set(self.config.get("text_model", "glm-4-flash"))
        
        r4 = ctk.CTkFrame(s, fg_color="transparent")
        r4.pack(fill="x", pady=2)
        ctk.CTkLabel(r4, text="Vision Model:", width=90).pack(side="left")
        self.vision_model = ctk.CTkComboBox(r4, values=ZAIClient.VISION_MODELS, width=200)
        self.vision_model.pack(side="left", padx=10)
        self.vision_model.set(self.config.get("vision_model", "glm-4v-flash"))
    ...
    def _setup_tk(self):
        ...
        ttk.Label(s, text="Text Model:").pack(anchor="w", padx=10, pady=3)
        self.text_model = ttk.Combobox(s, values=ZAIClient.TEXT_MODELS, width=30)
        self.text_model.pack(anchor="w", padx=10)
        self.text_model.set(self.config.get("text_model", "glm-4-flash"))
        
        ttk.Label(s, text="Vision Model:").pack(anchor="w", padx=10, pady=3)
        self.vision_model = ttk.Combobox(s, values=ZAIClient.VISION_MODELS, width=30)
        self.vision_model.pack(anchor="w", padx=10)
        self.vision_model.set(self.config.get("vision_model", "glm-4v-flash"))
    ...
    def _save(self):
        ...
        self.config["text_model"] = self.text_model.get().strip()
        self.config["vision_model"] = self.vision_model.get().strip()
        self.config["system_prompt"] = self.prompt.get("1.0", "end").strip()
        self._save_config()
        self.bot.config = self.config
        self.ai.text_model = self.config["text_model"]
        self.ai.vision_model = self.config["vision_model"]
